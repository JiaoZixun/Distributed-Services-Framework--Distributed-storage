...
void Listen(EventVec &events) {
    ...
    for(auto& ev:events) {
        //SYLAR_LOG_INFO(g_logger) << "events 1111";
        sd->addServlet(ev.first, ev.second);
        //SYLAR_LOG_INFO(g_logger) << "events 2222";
    }
    // SYLAR_LOG_INFO(g_logger) << "1111";
    server->start();
    // SYLAR_LOG_INFO(g_logger) << "2222";
}
